...
public class IUserService implements UserService {
	...
	public List<UserResource> getUsers() {
		return userGateway.getUsers().stream().map(userMapper::mapModelToResource).collect(Collectors.toList());
	}
	...
	public void activateAccount(Long userId, UUID userRegistrationTokenId) {
		...
		UserRegistrationToken userRegistrationToken = userRegistrationTokenGateway
				.getUserRegistrationTokenById(userRegistrationTokenId);
	...
	private void sendRegistrationMail(String email, Long userId, UUID userRegistrationTokenId) {
		...
		message.setText("To activate your account click this link. http://localhost:8080/user/activate-account/"
				+ userId + "/" + userRegistrationTokenId);
